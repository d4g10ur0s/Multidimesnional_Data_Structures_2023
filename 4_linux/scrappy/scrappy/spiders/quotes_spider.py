...
class QuotesSpider(scrapy.Spider):
    ...
    def write_file(self,scientist):
        ...
        path = os.getcwd()
        try :
            if os.path.exists("/home/dag1/Desktop/4linux/data/scientists"):
                pass
            else:
                os.mkdir("/home/dag1/Desktop/4linux/data/scientists")

            f = open("/home/dag1/Desktop/4linux/data/scientists/"+scientist.loc[0]["name"]+".json","w+")
            scientist.to_json(f)
            #input('meta')
            f.close()
        except ValueError:
    ...
    async def parse_2(self,response):
        path = os.getcwd()
        f= open("/home/dag1/Desktop/4linux/scrappy/scrappy/spiders/words/words.txt","r")
        words = f.readlines()
        f.close()
        t = []
        ps = PorterStemmer()
